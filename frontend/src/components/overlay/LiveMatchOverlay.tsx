...
export const LiveMatchOverlay: Component<Props> = (props) => {
  ...
  return (
    <div class="w-full min-h-screen bg-[#080C12]/95 text-white font-sans p-4 md:p-6 flex flex-col gap-6 select-none border border-white/5 shadow-2xl">
      
      {/* TACTICAL OVERLAY COMMAND HEADER & STAGE SWITCHER */}
      <header class="w-full bg-[#111726]/90 backdrop-blur-xl border border-val-cyan/30 rounded-2xl p-4 shadow-[0_0_20px_rgba(0,229,255,0.15)] flex flex-col lg:flex-row items-center justify-between gap-4">
        <div class="flex items-center gap-3">
          <div class="w-3 h-10 bg-val-cyan rounded-full shadow-glow-cyan" />
          <div>
            <h1 class="text-xl md:text-2xl font-black font-tactical tracking-wider flex items-center gap-2">
              <span>VAL-METRICS</span>
              <span class="text-val-cyan">DESKTOP HUD</span>
            </h1>
            <p class="text-xs text-val-muted font-mono tracking-wide flex items-center gap-2">
              <span class="w-2 h-2 rounded-full bg-val-emerald animate-pulse" />
              <span>LCU LOOPBACK INTEGRATED</span> • <span class="text-val-gold font-bold">{selectedMap()} ({gameMode()})</span>
            </p>
          </div>
        </div>
        ...
        <div class="flex flex-wrap items-center bg-[#070A0F] p-1.5 rounded-xl border border-white/10 gap-1 text-xs font-tactical">
          <button
            ...
            class={`px-3 py-1.5 rounded-lg font-bold transition-all ${activeStage() === 'home' ? 'bg-val-cyan text-val-obsidian shadow-glow-cyan font-black' : 'text-val-muted hover:text-white'}`}
          >
            🏡 HOME (LOBBY)
          </button>
          <button
            onClick={() => { setActiveStage('agent_select'); setGameMode('Competitive'); }}
            class={`px-3 py-1.5 rounded-lg font-bold transition-all ${activeStage() === 'agent_select' ? 'bg-val-gold text-val-obsidian font-black' : 'text-val-muted hover:text-white'}`}
          >
            🎭 AGENT SELECT
          </button>
          <button
            onClick={() => { setActiveStage('live_team'); setGameMode('Competitive'); }}
            class={`px-3 py-1.5 rounded-lg font-bold transition-all ${activeStage() === 'live_team' ? 'bg-val-red text-white shadow-glow-red font-black' : 'text-val-muted hover:text-white'}`}
          >
            ⚔️ LIVE 5V5 MATCH
          </button>
          <button
            onClick={() => { setActiveStage('live_ffa'); setGameMode('Deathmatch'); }}
            class={`px-3 py-1.5 rounded-lg font-bold transition-all ${activeStage() === 'live_ffa' ? 'bg-purple-600 text-white shadow-[0_0_12px_rgba(147,51,234,0.5)] font-black' : 'text-val-muted hover:text-white'}`}
          >
            🎯 DEATHMATCH / FFA
          </button>
        </div>
      ...
      <Show when={activeStage() === 'live_team'}>
        <div class="flex flex-col gap-4 animate-fade-in max-w-6xl mx-auto w-full">
          
          {/* ALLY TEAM (BLUE) TABLE */}
          <div class="bg-[#0F1626]/95 rounded-3xl border border-val-cyan/40 p-5 shadow-[0_0_25px_rgba(0,229,255,0.1)] overflow-x-auto">
                  ...
                  {(player) => (
                    <tr class="hover:bg-white/5 transition-colors">
                      <td class="py-2.5 font-sans font-bold text-sm text-white flex items-center gap-2.5">
                        <img src={player.agentIcon} alt={player.agent} class="w-8 h-8 rounded-lg bg-black border border-white/20 object-cover" />
          ...
          <div class="w-full bg-[#080B12] px-6 py-3 rounded-2xl border border-white/10 flex flex-wrap items-center justify-between gap-4 shadow-xl">
            ...
            <div class="flex items-center gap-6 font-mono text-xs">
              <div class="flex items-center gap-2">
                <span class="px-2 py-0.5 rounded bg-val-cyan/20 text-val-cyan font-bold">1.06</span>
                <span class="text-val-muted text-[10px] uppercase font-bold">AVG K/D</span>
                <span class="px-2 py-0.5 rounded bg-rose-500/20 text-rose-400 font-bold">1.08</span>
              </div>
              <div class="flex items-center gap-2">
                <span class="px-2 py-0.5 rounded bg-val-emerald/20 text-val-emerald font-bold">54%</span>
                <span class="text-val-muted text-[10px] uppercase font-bold">AVG WIN RATE</span>
                <span class="px-2 py-0.5 rounded bg-rose-500/20 text-rose-400 font-bold">56%</span>
              </div>
              <div class="flex items-center gap-2">
                <span class="px-2 py-0.5 rounded bg-val-gold/20 text-val-gold font-bold">+7.2</span>
                <span class="text-val-muted text-[10px] uppercase font-bold">DDΔ/R ADVANTAGE</span>
          ...
          <div class="bg-[#18111A]/95 rounded-3xl border border-rose-500/40 p-5 shadow-[0_0_25px_rgba(244,63,94,0.1)] overflow-x-auto">
            <div class="flex items-center justify-between mb-3 pb-2 border-b border-white/10">
      ...
      <Show when={activeStage() === 'live_ffa'}>
        <div class="max-w-4xl mx-auto w-full bg-[#121624]/95 p-6 rounded-3xl border border-purple-500/40 shadow-[0_0_30px_rgba(147,51,234,0.15)] animate-fade-in overflow-x-auto">
          <div class="flex items-center justify-between mb-4 pb-3 border-b border-white/10">
            <div>
              <h3 class="text-lg font-black font-tactical text-purple-400 tracking-wider uppercase flex items-center gap-2">
                <span>🎯</span> FREE-FOR-ALL SCOREBOARD ({gameMode()})
              </h3>
              <p class="text-[11px] text-val-muted uppercase font-bold mt-0.5">
                No team segregation in Deathmatch & Escalation • Real-time Marksmanship
              </p>
            </div>
            <span class="px-3 py-1 rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/40 font-tactical font-black text-xs">
              10 PLAYERS IN LOBBY
            </span>
          </div>
          ...
          <table class="w-full text-left font-mono text-xs">
            <thead>
              <tr class="text-val-muted uppercase text-[11px] border-b border-white/10 pb-2">
                <th class="py-2">Rank / Agent</th>
                <th>Player ID</th>
                <th>VAL-Index</th>
                <th>K/D</th>
                <th>Win %</th>
            ...
            <tbody class="divide-y divide-white/5">
              <For each={[...allyTeam, ...enemyTeam]}>
                {(player, index) => (
                  <tr class="hover:bg-white/5 transition-colors">
                    <td class="py-3 font-sans font-bold text-sm text-white flex items-center gap-2.5">
                      <img src={player.agentIcon} alt={player.agent} class="w-9 h-9 rounded-xl bg-black border border-white/20 object-cover" />
                      <div>
                        <span class="block font-black text-purple-300">{player.rank}</span>
                        <span class="text-[10px] text-val-muted uppercase font-mono">{player.agent}</span>
                      </div>
                    </td>
                    <td class="font-bold text-white text-sm">{player.name}</td>
                    {player.isPrivate ? (
                      <td colSpan={6} class="text-center py-3 text-val-muted font-tactical italic tracking-wider bg-black/30 rounded-lg">
                        🔒 PROFILE SET TO PRIVATE
                      </td>
                    ) : (
                      <>
                        <td class="font-black font-tactical text-val-gold text-sm">{player.valIndex}</td>
                        ...
                        <td>
                          <span class="px-2 py-0.5 rounded bg-val-emerald/20 text-val-emerald text-[10px] font-bold uppercase">
                            VERIFIED
                          </span>
                        </td>
                      </>
